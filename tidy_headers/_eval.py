...
def literal_eval(i):
    ...
    try:
        ...
        if i == 'nan':
            ...
            warnings.warn("Reading in multidimensional arrays with nan, inf, -inf is not a guaranteed behavior.")
            string = i
            replacements = [('nan', 1e8, np.nan), ('inf', 1e9, np.inf), ('-inf', -1e9, -np.inf)] # not strictly safe
